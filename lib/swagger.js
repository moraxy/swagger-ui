...
SwaggerModel.prototype.getMockSignature = function(modelsToIgnore, secondLevel) {
  var propertiesStr = [];
  for (var i = 0; i < this.properties.length; i++) {
    prop = this.properties[i];

    if(prop.refModel != null)
        propertiesStr.push(prop.toString() + prop.refModel.getMockSignature([], true));
    else
  ...
  if(secondLevel)
    ...
    classOpen = strong + this.name + ' {';

  var classClose = strong + '}' + strongClose;
  var returnVal = classOpen + strongClose + '<div>' + propertiesStr.join('</div><div>') + '</div>' + classClose;
...
SwaggerModelProperty.prototype.toSampleValue = function(value) {
  var result;
  if (value === "integer") {
    result = (this.defaultValue || 0);
  } else if (value === "boolean") {
    result = (this.defaultValue || false);
  } else if (value === "double" || value === "number") {
    result = (this.defaultValue || 0.0);
...
var SwaggerOperation = function(nickname, path, method, parameters, summary, notes, type, responseMessages, resource, consumes, produces, authorizations) {
  ...
  for(var i = 0; i < this.parameters.length; i ++) {
    ...
    //***//
    if(typeof param.type === 'undefined' && param.$ref)
        param.type = this.resource.models[param.$ref];
...
SwaggerOperation.prototype["do"] = function(args, opts, callback, error) {
    ...
    if(param.paramType === 'header') {
      if(args[param.name])
        params.headers[param.name] = args[param.name];
                                       ...
                                       || param.paramType.toLowerCase() === 'json') //***
  ...
  //*** always POST
  req = new SwaggerRequest("POST", this.urlify(args), params, opts, callback, error, this);
  if (opts.mock != null) {
    return req;
  } else {
    return true;
...
var SwaggerRequest = function(type, url, params, opts, successCallback, errorCallback, operation, execution) {
  ...
  //***//
  body = { "jsonrpc": "2.0", "method": this.operation.nickname, "params" : {}, "id": _.random(1,100)};

  for(var i = 0; i < params.length; i++) {
    var param = params[i];
    if(param.paramType === "json")
      if(param.type === "integer")
        body.params[param.name] = Number.parseInt(this.params[param.name]);
      //else if(param.type === "dict")
      //***
      else
        body.params[param.name] = this.params[param.name];
  }

  KeysToObject(body.params);
...
ShredHttpClient.prototype.execute = function(obj) {
  ...
  var transform = function(response) {
    ...
    if(response.request.body)
      if(typeof response.request.body.body !== 'undefined')
        out.body = response.request.body.body;
    //***//
...
function KeysToObject(keys) {
    ...
    _.each(keys, function (val, key, list) {
        ...
        (i >= 0) && (key[i] === prefix[i]); --i)
            continue;

        if (i < 0) {
            if(!list.values)
                list.values = {};
            list.values[key.substring(8)] = val;
            delete(list[key]);
        }
    });
